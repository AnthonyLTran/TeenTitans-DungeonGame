...
public class Player extends GameCharacter {
    ...
    public int getMonsterOldHP() {
        return monsterOldHP;
    }

    public void setMonsterOldHP(int monsterOldHP) {
        this.monsterOldHP = monsterOldHP;
    }
            ...
            if (playerInventory.get(i).getItemName().toLowerCase().compareTo(itemName.toLowerCase()) == 0) {
                ...
                if(playerInventory.contains(currentItem)) {
                    if (currentItem.getItemType().compareTo("usable") == 0) {
                        ...
                        ((Usable) currentItem).setConsumed(true);
    ...
    public int attack() {
        ...
        //int oldHP = currentRoom.getMonster().getCurrentHP();
        Scanner keyboard = new Scanner(System.in);
        Monster monster = currentRoom.getMonster();
        //Random rand = new Random(monsterOldHP);
        //int randRange= oldHP/2;
        int roundNo= player.getRoundCount();

        System.out.println("================= Round " + roundNo
                + " =================");
        player.setRoundCount(roundNo+1);
        ...
        if (monster.getCurrentHP() <= 0) {
            System.out.println("Congratulations! You defeated the "+
                    currentRoom.getMonster().getMonsterName());
            for (String s : monster.getDefeatedDescription()) {
                System.out.println(s);
            }
            currentRoom.getMonster().setDefeated(true);
            String itemDropped = monster.getDroppedItem();
        ...
        System.out.println("The " +currentRoom.getMonster().getMonsterName()+" has " + monster.getCurrentHP() +
                " HP left.");
...
//            monsterDamage *= 2;
//        }
        System.out.println("The " +currentRoom.getMonster().getMonsterName()+" attacks you for " +
                monsterDamage + ".");
    ...
    public void examineMonster(String itemName){
        ...
        if (itemName.toLowerCase().compareTo("monster") == 0) {
            ...
            if(monster.isDefeated()){
                ...
                System.out.println("The HP of the monster is "+monster.getCurrentHP()+
...
public void searchCurrentRoom(String currentCommand){
    currentRoom=map.getRooms().get(getPlayerLocation());
    ...
    if(!inventory.isEmpty()){
        ...
        for(Item item : inventory){
            if(item.isAvailable()) {
                ...
                System.out.println("Currently there is no items available in this room." +
                        "Defeat the monster, you may get something.");
            }
        }
    }else{
    ...
    public void exploreCurrentRoom(String currentCommand ) {
        if(currentCommand.compareTo("search")==0) {
            searchCurrentRoom(currentCommand);
            return;
        }
...
//                }
    ...
    public boolean equals(Object o) {
        ...
        return playerID == player.playerID && monsterOldHP == player.monsterOldHP && roundCount == player.roundCount && currentMode == player.currentMode && Objects.equals(playerInventory, player.playerInventory) && Objects.equals(currentRoom, player.currentRoom) && Objects.equals(equippedItems, player.equippedItems) && Objects.equals(map, player.map);
    }

    @Override
    public int hashCode() {
        return Objects.hash(super.hashCode(), playerID, playerInventory, currentRoom, equippedItems, map, monsterOldHP, roundCount, currentMode);
    }
}